...
def _build_engineer_prompt(mode: str, race_json: str) -> str:
    schema = (
        ...
        "s{st=session_state,ty=session_type,ses=session_num,tr=time_remain_s_omit_if_placeholder,"
        "te=elapsed_s,lt=laps_total,ot=is_on_track,ig=is_in_garage,at=air_temp_F,tt=track_temp_F,atc=air_C,ttc=track_C,"
        "wn=wetness,cls=car_class,flb=flags{yel,cau,grn,pcl=pit_lane_closed},pto=pit_lane_penalty_s}; "
        "m{l=lap,lr=laps_remain,p=pos,ful=fuel_liters,fu=fuel_USgal,fup=fuel_pct,fbl=fuel_burn_last_lap_USgal,"
        "fbh=fuel_burn_lap_history_USgal,psf=pit_service_fuel_sched_USgal,pte=pit_lane_time_session_s,"
        "fph=fuel_burn_lb_per_hr_SDK_scaled,fpe=fuel_USgal_per_lap_ema_if_present,fpl=fuel_USgal_per_lap_est,fcq=fuel_est_quality_hi|med|low,"
        ...
        "pb=pit_payback_laps,pw=pit_window_open,pwu=laps_until_window,tw,tws,twsl,twr,cmp=tire_compound,sp=speed_mph}; "
        "r{pl=pit_loss_sec,ts=tire_sets_avail,fc=fuel_tank_USgal_capacity,ftl=laps_per_full_tank_fuel_est}; "
        "rv=rivals; f=field; "
        "fi{dist=lap_dist_pct_tracked,f2=CarIdxF2Time_tracked,surf=track_surface_codes,hd=herd{apa,apb,isa,isb,pra,prb},"
        "rej=projected_reentry{CLEAN|TRAFFIC|PACK,n,dp},cpi=caution_pit{lrk,lda,ll,tl,xp,gr,pra,prb}}; "
        "twl=pit_box_tire_wear_log[{l,tt,at,LF,RF,LR,RR}]. "
        "h{receiver_history_only:ss,pl,pf,lr,ev,tr,al,fbh,twh,rej}. "
    )
    base = (
    ...
    if mode == "strategy":
        return (
            ...
            + "If h present, use h.pl/h.pf and s.ty/s.lt for realistic stop count; h.al avoids contradicting earlier plan. "
    ...
    return (
        ...
        + "Use m.fs and s.flb (yel/cau/grn/pcl pit-lane-closed) — if s.flb.pcl true, do NOT call PIT NOW. "
        + "If flag_state is CAUTION: default to STAY OUT unless fuel requires a stop or pitting gains clear track position; "
        + "use fi.hd.pra/prb pitting ratios and fi.hd.apa/apb herd counts. "
        + "Caution pits: fi.cpi.tl=total class spots lost on screen, fi.cpi.ll=lead-lap spots lost, fi.cpi.lda=lap-down "
        + "cars ahead that may stay out, fi.cpi.xp=projected on-track class position, fi.cpi.gr=restart grid rows back — "
        + "lap-down traffic can inflate tl vs ll; recommend pits using ll/gr not raw track order alone. "
        + "If flag_state is GREEN or UNKNOWN: do NOT recommend pitting unless we are inside the pit window or fuel requires it. "
        + "Fuel: prefer m.fbl (lap-boundary burn) and m.fbh trend over instantaneous fph when fcq=hi; m.psf is scheduled pit add; "
        + "m.fup is tank %. "
        + "Rejoin: fi.rej.v CLEAN=clear air, TRAFFIC=one car, PACK=multiple — factor into pit timing vs r.pl. "
        + "Use tires: tire_wear_last_known is a baseline from the last pit; twl/h.twh maps wear vs s.ttc/s.atc at prior stops. "
        + "Project next-stop wear using twsl and twr along with stint m.sl (stale wear when m.tws). "
        + "If h is present (engineer PC race memory): use h.ss for caution/green-run/pit context, h.lr/h.pl for pace "
        + "over multiple laps, h.tr/h.fbh for fuel trends, h.ev for recent cautions/pits/position swings, "
        + "h.rej for rejoin history, h.twh for wear-vs-temp, and h.al to stay consistent with prior calls unless telemetry clearly changed. "
        + "s.at/s.tt/s.atc/s.ttc/s.wn help tire deg and grip; m.cmp is compound when set. "
        + "Compare m.ga/m.gb to r.pl and fi.dist for undercut/overcut; crossover m.fo vs m.pb. "
